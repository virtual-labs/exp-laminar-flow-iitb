...
var start_button: HTMLButtonElement = <HTMLButtonElement>(
...
function load_higlighted_images() {
	...
	a1_labels = [
		...
		new Chemistry.Text(
			'Test Section',
			new Chemistry.Point(1100, 600),
			canvas
		),

		new Chemistry.Text(
			'Horizontal Pipe',
			new Chemistry.Point(200, 850),
			canvas
		),
		...
		new Chemistry.Text(
			'Temp. Controller',
			new Chemistry.Point(700, 850),
			canvas
		),

		new Chemistry.Text(
			'Temp. Indiciator',
			new Chemistry.Point(380, 650),
			canvas
		),
	];
}
...
function a1_display_current_question() {
	...
	display_score = new Chemistry.Text(
		`Score: ${total_score}/27`,
		new Chemistry.Point(1650, 620),
		canvas
	);
...
function display_message(text: string) {
	guide.disabled = false;
	guide.value = text;
	guide.disabled = true;
}

function add_to_content(text: string) {
...
function add_button(text: string) {
