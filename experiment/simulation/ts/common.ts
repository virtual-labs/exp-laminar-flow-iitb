...
function calculate_area() {
    ...
    area_of_inner_tube = Math.PI/4*(selected_dia**2)/10000;

    equivalent_diameter = (selected_shell_inside_dia - selected_outside_dia)/100;

    annulus_area = Math.PI/4*(selected_shell_inside_dia**2 - selected_outside_dia**2)/10000;
...
function a7_calculate_all(time: number, tci: number, tco: number, ti: number, to: number) {

    let res = []

    let nue = selected_volume*(10**(-6))/time;
    res.push(nue);
...
function a7_calculate_for_table_2() {
    ...
    let nuc = (0.023 * (rec**(0.8)) * (prc**(0.4)) * (selected_shell_inside_dia/selected_outside_dia)**(0.45));
...
function a7_calculate_for_table_3(U:number, u:number) {
    ...
    let re = (u * selected_dia * (10**(-2)) * rhoh)/mueh;

    res.push(re);

    let hi_st = kh/(selected_dia * (10**(-2)))*(1.86 * re**(1/3) * prh**(1/3) * (selected_dia/selected_length)**(1/3));
...
function random(min_val: number, max_val: number) {
